...
mean = np.load('mean_npy.npy')
std = np.load('std_npy.npy')
#LABELS = ['unknown','down','right','no','yes','up','go','off','on','silence','stop','left']

# For time axis
y = np.linspace(0,1,15976) # Depends on sample rate
model = load_model('models/best_new.h5')
model.load_weights('models/best_new.h5')
...
class RecordApp():
    ...
    def __init__(self,master):
        ...
        # lbl = graph, lbl2 = command list, lbl3 = "Prediction:" text, lbl4 = prediction
        ...
        self.num1.grid(row = 1, column = 1, sticky = "w")
        ...
        self.num3.grid(row = 3, column = 1, sticky = "w") 
        
        self.num4 = Label(main, text="4. yes", font=("Helvetica", 15))
        self.num4.grid(row = 4, column = 1, sticky = "w")

        self.num5 = Label(main, text="5. up", font=("Helvetica", 15))
        self.num5.grid(row = 5, column = 1,  sticky = "w")
        
        self.num6 = Label(main, text="6. go", font=("Helvetica", 15))
        self.num6.grid(row = 6, column = 1, sticky = "w")
        
        self.num7 = Label(main, text="7. off", font=("Helvetica", 15))
        self.num7.grid(row = 7, column = 1, sticky = "w")
        
        self.num8 = Label(main, text="8. on", font=("Helvetica", 15))
        self.num8.grid(row = 8, column = 1, sticky = "w")

        self.num9 = Label(main, text="9. stop", font=("Helvetica", 15))
        self.num9.grid(row = 9, column = 1, sticky = "w")

        self.num10 = Label(main, text="10. left", font=("Helvetica", 15))
        self.num10.grid(row = 10, column = 1, sticky = "w")
        
        self.lbl3 = Label(main, text="Prediction: ", font=("Helvetica", 20))
        self.lbl3.grid(row = 12 , column =0, sticky = "nsew")
        self.lbl4 = Label(main, text = "       ", font=("Helvetica", 20))
        self.lbl4.grid(row = 12, column =1, sticky = "w")
        self.predtime = Label(main, text = "Time taken (sec): ", font = ("Helvetica", 20))
        self.predtime.grid(row = 13, column =0, sticky = "nsew")
        self.time = Label(main, text = "    ", font=("Helvetica", 20))
        self.time.grid(row = 13, column =1, sticky = "w")
        ...
        main.bind("<Return>",self.startrecording)
        self.button.grid(row = 11, column = 0,columnspan=2, sticky = "nsew")
        self.LABELS = ['stop','unknown','off','on','go','silence','down','left','right','up','yes','no']  
    ...
    def _record(self):
        ...
        for i in range(0, int(self.RATE / self.CHUNK * self.RECORD_SECONDS)):
            stream = p.open(format=self.FORMAT,channels=self.CHANNELS,rate=self.RATE,input=True,input_device_index=0,frames_per_buffer=self.CHUNK)
...
#        data = librosa.effects.time_stretch(data, 0.9)
